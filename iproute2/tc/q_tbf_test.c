...
#include "tc_tbf_test.h"

static void explain(void)
{
	fprintf(stderr,
		"Usage: ... tbf_test limit BYTES burst BYTES[/BYTES] rate KBPS [ mtu BYTES[/BYTES] ]\n"
...
{
	fprintf(stderr, "tbf_test: illegal value for \"%s\": \"%s\"\n", arg, val);
}


static int tbf_test_parse_opt(struct qdisc_util *qu, int argc, char **argv,
			 struct nlmsghdr *n, const char *dev)
{
	...
	while (argc > 0) {
		if (matches(*argv, "limit") == 0) {
			NEXT_ARG();
			if (opt.limit) {
				fprintf(stderr, "tbf_test: duplicate \"limit\" specification\n");
				return -1;
			}
			if (latency) {
				fprintf(stderr, "tbf_test: specifying both \"latency\" and \"limit\" is not allowed\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (latency) {
				fprintf(stderr, "tbf_test: duplicate \"latency\" specification\n");
				return -1;
			}
			if (opt.limit) {
				fprintf(stderr, "tbf_test: specifying both \"limit\" and \"/latency\" is not allowed\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (buffer) {
				fprintf(stderr, "tbf_test: duplicate \"buffer/burst/maxburst\" specification\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (mtu) {
				fprintf(stderr, "tbf_test: duplicate \"mtu/minburst\" specification\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (mpu) {
				fprintf(stderr, "tbf_test: duplicate \"mpu\" specification\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (rate64) {
				fprintf(stderr, "tbf_test: duplicate \"rate\" specification\n");
				return -1;
			}
			if (strchr(*argv, '%')) {
			...
			NEXT_ARG();
			if (prate64) {
				fprintf(stderr, "tbf_test: duplicate \"peakrate\" specification\n");
				return -1;
			}
			if (strchr(*argv, '%')) {
			...
			NEXT_ARG();
			if (overhead) {
				fprintf(stderr, "tbf_test: duplicate \"overhead\" specification\n");
				return -1;
			}
		...
		} else if (matches(*argv, "param") == 0) {
			NEXT_ARG();
			if (get_u32(&opt.param, *argv, 10)) {
				explain1("param", *argv);
				return -1;
			}
			...
			fprintf(stderr, "tbf_test: unknown parameter \"%s\"\n", *argv);
			explain();
			return -1;
		}
		argc--; argv++;
	 ...
	 */
	if (rate64 == 0) {
		fprintf(stderr, "tbf_test: the \"rate\" parameter is mandatory.\n");
		verdict = -1;
	}
	if (!buffer) {
		fprintf(stderr, "tbf_test: the \"burst\" parameter is mandatory.\n");
		verdict = -1;
	}
	if (prate64) {
		if (!mtu) {
			fprintf(stderr, "tbf_test: when \"peakrate\" is specified, \"mtu\" must also be specified.\n");
			verdict = -1;
		}
	}

	if (opt.limit == 0 && latency == 0) {
		fprintf(stderr, "tbf_test: either \"limit\" or \"latency\" is required.\n");
		verdict = -1;
	}
	...
	if (tc_calc_rtable(&opt.rate, rtab, Rcell_log, mtu, linklayer) < 0) {
		fprintf(stderr, "tbf_test: failed to calculate rate table.\n");
		return -1;
	}
	...
	if (opt.peakrate.rate) {
		...
		if (tc_calc_rtable(&opt.peakrate, ptab, Pcell_log, mtu, linklayer) < 0) {
			fprintf(stderr, "tbf_test: failed to calculate peak rate table.\n");
			return -1;
		}
...
static int tbf_test_print_opt(struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
...
struct qdisc_util tbf_test_qdisc_util = {
	.id		= "tbf_test",
	.parse_qopt	= tbf_test_parse_opt,
	.print_qopt	= tbf_test_print_opt,
};
