...
{
	fprintf(stderr,
		"Usage: ... tdma limit BYTES burst BYTES[/BYTES] rate KBPS [ mtu BYTES[/BYTES] ]\n"
		"	[ peakrate KBPS ] [ latency TIME ] "
		"[ overhead BYTES ] [ linklayer TYPE ] [ frame NANOSECONDS ] [ slot NANOSECONDS ]\n");
}

static void explain1(const char *arg, const char *val)
{
	fprintf(stderr, "tdma: illegal value for \"%s\": \"%s\"\n", arg, val);
}


static int tdma_parse_opt(struct qdisc_util *qu, int argc, char **argv,
			 struct nlmsghdr *n, const char *dev)
{
	...
	while (argc > 0) {
		if (matches(*argv, "limit") == 0) {
			NEXT_ARG();
			if (opt.limit) {
				fprintf(stderr, "tdma: duplicate \"limit\" specification\n");
				return -1;
			}
			if (latency) {
				fprintf(stderr, "tdma: specifying both \"latency\" and \"limit\" is not allowed\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (latency) {
				fprintf(stderr, "tdma: duplicate \"latency\" specification\n");
				return -1;
			}
			if (opt.limit) {
				fprintf(stderr, "tdma: specifying both \"limit\" and \"/latency\" is not allowed\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (buffer) {
				fprintf(stderr, "tdma: duplicate \"buffer/burst/maxburst\" specification\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (mtu) {
				fprintf(stderr, "tdma: duplicate \"mtu/minburst\" specification\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (mpu) {
				fprintf(stderr, "tdma: duplicate \"mpu\" specification\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (rate64) {
				fprintf(stderr, "tdma: duplicate \"rate\" specification\n");
				return -1;
			}
			if (strchr(*argv, '%')) {
			...
			NEXT_ARG();
			if (prate64) {
				fprintf(stderr, "tdma: duplicate \"peakrate\" specification\n");
				return -1;
			}
			if (strchr(*argv, '%')) {
			...
			NEXT_ARG();
			if (overhead) {
				fprintf(stderr, "tdma: duplicate \"overhead\" specification\n");
				return -1;
			}
		...
		} else if (matches(*argv, "frame") == 0) {
			NEXT_ARG();
			if (get_time64(&opt.frame, *argv)) {
				explain1("frame", *argv);
				return -1;
			}
		} else if (matches(*argv, "slot") == 0) {
			NEXT_ARG();
			if (get_time64(&opt.slot, *argv)) {
				explain1("slot", *argv);
				return -1;
			}
			...
			fprintf(stderr, "tdma: unknown parameter \"%s\"\n", *argv);
			explain();
			return -1;
		}
		argc--; argv++;
	 ...
	 */
	if (rate64 == 0) {
		fprintf(stderr, "tdma: the \"rate\" parameter is mandatory.\n");
		verdict = -1;
	}
	if (!buffer) {
		fprintf(stderr, "tdma: the \"burst\" parameter is mandatory.\n");
		verdict = -1;
	}
	if (prate64) {
		if (!mtu) {
			fprintf(stderr, "tdma: when \"peakrate\" is specified, \"mtu\" must also be specified.\n");
			verdict = -1;
		}
	}

	if (opt.limit == 0 && latency == 0) {
		fprintf(stderr, "tdma: either \"limit\" or \"latency\" is required.\n");
		verdict = -1;
	}
	...
	if (tc_calc_rtable(&opt.rate, rtab, Rcell_log, mtu, linklayer) < 0) {
		fprintf(stderr, "tdma: failed to calculate rate table.\n");
		return -1;
	}
	...
	if (opt.peakrate.rate) {
		...
		if (tc_calc_rtable(&opt.peakrate, ptab, Pcell_log, mtu, linklayer) < 0) {
			fprintf(stderr, "tdma: failed to calculate peak rate table.\n");
			return -1;
		}
...
static int tdma_print_opt(struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
	...
	print_int(PRINT_ANY, "frame", "frame %lld ", qopt->frame);
	print_int(PRINT_ANY, "slot", "slot %lld ", qopt->slot);

	return 0;
}

struct qdisc_util tdma_qdisc_util = {
	.id		= "tdma",
